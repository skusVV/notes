...
export class ClassifierService {
  ...
  private buildInstruction(context: ClassifierContext): string {
    ...
    const parts = [
      ...
      '',
      'Symptoms. A symptom is a health event the user is experiencing. Fill the symptom object.',
      'Rules for its fields:',
      '- startedAt: resolve WHEN the symptom began against the current local time above, and emit it',
      '  as ISO 8601 with the user\'s local offset. When the message states a start ("з понеділка",',
      '  "вчора ввечері", "2 години тому"), resolve that to startedAt.',
      '- YEAR anchor: startedAt uses the SAME YEAR as the current local time above unless the message',
      '  itself names a different year. A relative past reference - "вчора"/"yesterday", "2 години',
      '  тому", "з понеділка", "минулого тижня" - stays inside the current year; "вчора"/"yesterday"',
      '  is always the calendar day immediately before now and NEVER crosses back into the previous',
      '  year. Take the year from "now", not from your own training-time default - the day and month',
      '  are what shift, the year does not unless it was stated.',
      '- When the symptom is described in the PRESENT TENSE with no stated start ("болить голова",',
      '  "my head hurts"), set startedAt to NOW - the current local time above. A present-tense',
      '  symptom is happening now. This is the ONLY inference allowed here.',
      '- When the message is about a PAST symptom but gives no resolvable time, OMIT startedAt. Never',
      '  fabricate a start for a past symptom whose time you cannot resolve.',
      '- NEVER infer severity. Emit it ONLY when the user states a number ("на 7 з 10",',
      '  "severity 7"). An intensity word - "дуже", "сильно", "bad", "terrible" - is NOT a number and',
      '  must leave severity absent. One fabricated 7 pollutes every later average.',
      '- durationMinutes ONLY when the user states how long it lasted.',
      '- type is an English snake_case slug, reusing a known slug when one fits (the known-slug list',
      '  is appended below when non-empty).',
      'Examples (now = 2026-09-16 09:00 local):',
      '- "болить голова" -> symptom, type "headache", startedAt = 2026-09-16T09:00 local, no severity.',
      '- "дуже сильно болить голова" -> symptom, type "headache", NO severity (intensity is not a',
      '  number), startedAt = now.',
      '- "болить голова, десь на 7 з 10" -> symptom, type "headache", severity 7, startedAt = now.',
      '- "вчора ввечері боліла голова" -> symptom, type "headache", startedAt = 2026-09-15 evening',
      '  local - the day before now, and still 2026 (the year of "now"), NOT 2025. No severity.',
      '- "yesterday evening my head hurt" -> same as above: startedAt = 2026-09-15 evening local, year',
      '  2026 taken from "now", never a previous year.',
      '',
